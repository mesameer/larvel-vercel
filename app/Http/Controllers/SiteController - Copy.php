...
class SiteController extends Controller
{
    ...
    public function index(Request $request) { 
        ...
        $allTemplatesRow = Template::on('onthefly')
                        ...
                        ->first();
        if(empty($this->custom_keyword) || empty($this->location)) {
            if(empty($this->custom_keyword)) {
                if(!empty($site->service_id)) {
                    $this->custom_keyword  =$site->service_id;
                } else {
                    echo "<h1>Please Select Default Services.</h1>";
                }
            }
            if(empty($location)) {
                if(!empty($site->city)) {
        ...
        $allServicesList = $this->getServicesList($this->custom_keyword,$this->location,$this->type);
        ...
        if($this->type == 'zip' && !empty($this->servicePage)) {
			$keyword_location_url=$this->domain.'/'.makeUrl($zip)."/".makeUrl($this->custom_keyword);
			$keywordAnchorTag = '<a style="color:white;text-decoration:underline" href='.$this->domain.'/'.makeUrl($this->custom_keyword).'> our '.$this->custom_keyword.' Company</a>';
		}  else if($this->type == 'zip') {
			$keyword_location_url=$this->domain.'/'.makeUrl($zip);
			$keywordAnchorTag = '<a style="color:white;text-decoration:underline" href='.$this->domain.'> our '.$this->custom_keyword.' company</a>';
		} else if($this->type == 'service') {
			$keyword_location_url=$this->domain.'/'.makeUrl($this->custom_keyword);
		}  
		$replaceKey=[ucwords($meta_heading),$meta_title,$meta_description,
			ucwords($this->custom_keyword),ucwords($this->location),$this->phone,$keyword_location_url,$this->latitude,$this->longitude,$this->text_block_1,$this->text_block_2,$this->text_block_3,$this->text_block_4,ucwords($this->state),ucwords($this->county),ucwords($this->city),json_encode($zipcodes),json_encode($allServicesList),$keyword_location_url,$this->stateCodeValue
		];		
		$template=str_replace($searchKey,$replaceKey,$allTemplatesRow->html);
        echo "<pre>";print_r($template);die;
    }


    public function getServicesList($custom_keyword,$location,$type) {
		...
		foreach ($ralated_service as $key =>  $service)	{
            ...
            if($type =='zip') {
                $url = $this->makeUrl($zip)."/".makeUrl($service);
            } else {
    ...
    function getZipcodesList($state,$city,$custom_keyword,$servicePage) {
		...
		foreach ($allZipCode as $zip){
			if(!empty($servicePage)) {
				$url= $this->makeUrl($zip)."/".makeUrl($custom_keyword);
			} else {
